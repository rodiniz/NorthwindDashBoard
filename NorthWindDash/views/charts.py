...
class StatsState(rx.State):
    ...
    def load_data_frames(self):
      ...
      with rx.session() as session:
        ...
        self._df_orders = pd.read_sql("SELECT  * from orders",_connection)

        _df = pd.read_sql('select * from order_details',_connection)

        self.df_merged= _df.merge(self._df_orders)
        ...
        self.df_order_details = pd.read_sql('SELECT o.order_date, od.order_id,p.product_name, od.quantity, od.unit_price, od.discount '
                                            ...
                                            ' join orders o on od.order_id = o.order_id',_connection)       
    ...
    def load_data(self):
        ...
        self._df_orders['YearMonth']= pd.to_datetime(self._df_orders['order_date']).dt.strftime('%Y-%m')

        self.order_months=self._df_orders['YearMonth'].unique().tolist()

        self.order_month_selected=self._df_orders.iloc[0]['YearMonth']      
